...
logo <- function(pagebreak = TRUE) {
  ...
  shiny::div(
    class = paste("print-header", ifelse(pagebreak, "page-header", "")),
    shiny::div(
      class = "left-logo",
      shiny::img(
...
p <- ggplot() +
  geom_sf(data = all_boundaries_in_bbox, colour = "grey", fill = NA) +
  geom_sf(data = region_boundaries, colour = "black", fill = "white", alpha = 0.75) +
  theme_void()
...
regions_text <- glue::glue("Selected census {level_text}{plural}: {regions}",
  plural = ifelse(length(regions) > 1, "s", ""),
  regions = paste0(regions, collapse = ", ")
)

shiny::column(
  width = 12,
  shiny::p(regions_text)
)
```
...
if (!str_starts(population_change_formatted, "-")) {
  population_change_formatted <- paste0("+", population_change_formatted)
}
