...
params:
  ...
  geo_uid: !r c("9330042.00", "9330041.02", "9330041.01", "9330040.02", "9330039.02",  "9330039.01", "9330038.00", "9330037.02", "9330037.01", "9330034.02",  "9330034.01", "9330033.02", "9330033.01", "9330032.02", "9330032.01",  "9330031.02", "9330031.01", "9330030.00", "9330029.00", "9330028.00",  "9330027.02", "9330027.01", "9330026.00", "9330023.00", "9330022.00",  "9330021.00", "9330020.00", "9330019.00", "9330018.02", "9330018.01",  "9330014.02", "9330014.01", "9330013.04", "9330013.03", "9330013.01",  "9330012.02", "9330012.01", "9330010.01", "9330009.00", "9330008.02")
  # level: "csd"
  level: "ct"
...
vectors_data <- vectors_data %>%
  mutate(geo_uid = fct_expand(geo_uid, regions))
...
age_cohorts <- data_breakdown %>%
  ...
  mutate(label = fct_reorder(label, label))
...
indigenous_identity <- data_breakdown %>%
  filter(
    label_short == "indigenous_identity",
    vector != highest_parent_vector
  ) %>%
  select(-value, value = value_proportion) %>%
  pull_and_format_value()
...
shiny::column(
  width = 3,
  snapshot_card("Indigenous identity", indigenous_identity)
)
```
...
data_export <- bind_rows(
  ...
  data_export %>% filter(label_short == "income_buckets") %>% mutate(vector = paste(vector, "($20,000 buckets)")) %>%
    mutate(breakdown = fct_relevel(breakdown, levels(total_income[["label"]]))) %>% arrange(breakdown),
  ...
  data_export %>% filter(label_short %in% c("shelter_cost_owner", "shelter_cost_renter")) %>% mutate(vector = "Average monthly shelter cost") %>% mutate(value = round(value, 2)),
  # Age (5 year buckets)
  data_export %>%
  ...
  data_export %>% filter(label_short == "household_type") %>%
    mutate(breakdown = fct_relevel(breakdown, levels(multiple_family_households[["label"]]))) %>% arrange(breakdown)
) %>%
