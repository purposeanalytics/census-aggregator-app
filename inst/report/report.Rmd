...
```{r map, fig.height = 3.5, dpi = 300, fig.alt = "A map showing the outlines of the selected geographies."}
...
language_at_home <- data_breakdown %>%
  ...
  inner_join(vectors %>%
    filter(label_short == "language_at_home", str_detect(details, "Single")) %>%
...
visible_minority_breakdown <- data_breakdown %>%
  filter(
    label_short == "visible_minority",
    vector != highest_parent_vector
  ) %>%
...
data_export <- bind_rows(
  ...
  ethnic_origin %>% rename(breakdown = label) %>% mutate(vector = "Top 10 ethnic or cultural origins"),
