...
```{r data}
regions <- c("3520005", "3521005", "3521010")
level <- "CSD"
dataset <- glue::glue("extdata/{geography}_values", geography = tolower(level))

data <- open_dataset(app_sys(dataset))
...
data_breakdown <- vectors_data_filtered %>%
  ...
  distinct() %>%
  left_join(vectors, by = c("highest_parent_vector", "vector", "type", "label", "units", "parent_vector", "aggregation", "details")) %>%
